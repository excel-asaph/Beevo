...
async function simulateUser(index: number, state: { hash: string, variantId: string }) {
    ...
    await sendSyntheticEvent(sessionId, state.hash, state.variantId, 'proof_dwell_summary', { meta: { dwell_ms: dwellMs } });
}

async function main() {
    console.log("🚀 Starting SMART Traffic Simulation for Proof Block...");

    // Dynamically resolve the LIVE Proof Block ID
    const state = await getLiveState('proof');
