            ...
            <header className="mb-8 flex justify-between items-center border-b border-neutral-800 pb-4">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono text-neutral-500">SYSTEM {config.hitl.enabled ? 'ONLINE' : 'OFFLINE'}</span>
                    </div>
                </div>
            </header>
                                                ...
                                                {Object.entries(config.sections[sectionKey]).map(([k, v]) => {
                                                    ...
                                                    return (
                                                        <div key={k} className="flex justify-between items-center text-xs">
                                                            ...
                                                            <input
                                                                type="number"
                                                                className="w-16 bg-neutral-800 text-white font-bold px-2 py-1 rounded text-center border border-transparent focus:border-blue-500 outline-none"
                                                                value={String(v)}
                                                                onChange={(e) => handleMetricChange(sectionKey, k, e.target.value)}
                                                                onBlur={(e) => saveMetric(sectionKey, k, parseFloat(e.target.value))}
                                                            />
                                                        </div>
                                                    );
                                                })}
